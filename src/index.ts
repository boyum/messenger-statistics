...
const filesInConversation = fs.readdirSync(
  join(__dirname, "..", "inbox", conversationName),
);
const files = filesInConversation.filter(fileName =>
...
async function readFiles(files: string[]) {
  ...
  const conversations: FBConversation[] = await Promise.all(
    files
      .map(fileName =>
        join(__dirname, "..", "inbox", conversationName, fileName),
      )
      .map(filePath => readFile(filePath))
      .map(async fileBuffer => await fileBuffer)
      .map(async fileContentsBufferPromise =>
        (await fileContentsBufferPromise).toString("utf-8"),
      )
  ...
  const messages: FBMessage[] = conversations.flatMap(
  ...
  if (countedWords.length) {
    ...
    console.info(
      `Number of times notable words were mentioned:\n${formatCountedWords(
        wordCount,
        false,
      )}\n`,
    );
  ...
  const participantMessageCount = countMessagesFromParticipants(
    messages,
    participants,
  );
  console.info(
    `Number of messages from participants:\n${formatCountedWords(
  ...
  console.info(
    `Number of messages from participants with photos:\n${formatCountedWords(
  ...
  console.info(
    `Number of messages from participants with videos:\n${formatCountedWords(
  ...
  console.info(
    `Number of messages from participants with audio messages:\n${formatCountedWords(
  ...
  console.info(
    `Number of times participants have shouted:\n${formatCountedWords(
      participantShoutMessageCount,
    )}\n`,
  );
...
function formatCountedWords(
  ...
  const countedWordsFormatted = Object.entries(words)
  ...
  return messages
    ...
    .reduce((countedWords, messageContent) => {
      for (const word of words) {
        const foundWordInMessageContent = messageContent
          .toLowerCase()
          .includes(word);
...
function countShouts(
  ...
  const messagesWithOnlyShouts = messages.filter(
    message => message.content === "\u00f0\u009f\u0097\u00a3",
  );
